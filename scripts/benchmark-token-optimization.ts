...
import type { TaskListParams } from '../src/mcp/types';
...
async function runTest() {
  ...
  for (const testCase of testCases) {
    ...
    const hasContent =
      tasks.length > 0 && tasks.some((task) => task.content && task.content.trim().length > 0);
  ...
  const fullResponseSize =
    results.find((r) => r.params.include_content === true && r.params.include_completed === true)
      ?.responseSize || 0;

  const defaultResponseSize =
    results.find((r) => !r.params.include_content && !r.params.include_completed)?.responseSize ||
    0;

  if (fullResponseSize > 0 && defaultResponseSize > 0) {
    const reductionPercentage = (
      ((fullResponseSize - defaultResponseSize) / fullResponseSize) *
      100
    ).toFixed(2);
    console.log(
  ...
  const defaultResult = results.find(
    (r) => !r.params.include_content && !r.params.include_completed
  );
  ...
  const fullResult = results.find(
    (r) => r.params.include_content === true && r.params.include_completed === true
  );

  // Only verify completed tasks were included if there are any completed tasks in the full result
  if (fullResult?.completedTasksIncluded) {
    assertTest(
      completedResult?.completedTasksIncluded,
      'include_completed=true correctly includes completed tasks'
    );
