...
import type { Task, TaskListFilter } from '../../lib/types';
...
export function TaskListViewInner() {
  ...
  const typeOptions = useMemo(() => [...new Set(tasks.map((task) => task.type))].sort(), [tasks]);
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter((task) => {
      ...
      if (filters.feature && (!task.subdirectory || task.subdirectory !== filters.feature))
        return false;
