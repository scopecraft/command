...
export function ColumnToggle<T extends Task>({ table }: ColumnToggleProps<T>) {
  ...
  const toggleableColumns = table.getAllColumns().filter(
    (column) => column.getCanHide() && column.id !== 'actions' && column.id !== 'title' // Always keep title column visible
  );

  return (
    <div className="relative">
      <Button variant="outline" size="sm" onClick={() => setIsOpen(!isOpen)} className="ml-2">
        Columns
      </Button>
