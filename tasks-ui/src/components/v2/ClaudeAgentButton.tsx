...
export function ClaudeAgentButton({ onClick, className, taskId }: ClaudeAgentButtonProps) {
  ...
  if (sessionActive) {
    return (
      <Button variant="secondary" disabled className={cn('uppercase', className)}>
  ...
  if (isStarting) {
    return (
      <Button variant="atlas" disabled className={cn('uppercase', className)}>
        Starting...
      </Button>
    );
  }
  ...
  return (
    <Button variant="atlas" onClick={handleClick} className={cn('uppercase group', className)}>
      Start Claude Agent
