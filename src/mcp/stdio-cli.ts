  ...
  .action(async (options) => {
    ...
    if (options.rootDir) {
      try {
        ...
        console.error(`Using project root from CLI: ${options.rootDir}`);
      } catch (error) {
        console.error(
    ...
    if (options.config) {
      ...
      console.error(`Using config file: ${options.config}`);
    }

    // Handle mode option - now this is handled by configuration manager
    if (options.mode) {
      console.error(`Note: Mode option is deprecated. Project configuration is now managed automatically.`);
    }
    ...
    if (!projectConfig.validateEnvironment()) {
      ...
      try {
        console.error(`Creating project structure...`);
        projectConfig.initializeProjectStructure();
        console.error(`Project structure created successfully.`);
      } catch (error) {
        console.error(
    ...
    console.error(`Starting STDIO MCP server in ${modeText} mode...`);
