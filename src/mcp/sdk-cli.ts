...
/**
 * SDK MCP server CLI
 * Command-line interface for starting the MCP server with the SDK implementation
 */
import { Command } from 'commander';
import { startSdkServer } from './sdk-server.js';
import fs from 'fs';
...
program
  .name('scopecraft-command-mcp-sdk')
  .description('MCP server for Markdown-Driven Task Management (MDTM) using the SDK implementation')
  .version(version)
  ...
  .action(async (options) => {
    ...
    console.log(`Starting SDK MCP server in ${modeText} mode...`);

    await startSdkServer(options.port, {
