...
import { ScopecraftSessionStorage, type ScopecraftSessionMetadata } from './session-storage.js';
...
export async function getSessionDetails(taskId: string): Promise<SessionDetails | null> {
  ...
  const session = sessions.find(s => 
    s.scopecraftData?.taskId === taskId
  );
          ...
          for (const event of events.slice(-20)) {
            ...
            if (streamParser.isToolUseEvent(event)) {
              logEntries.push({
                ...
                taskId,
                content: `🔧 Using tool: ${(event as any).tool}`,
                type: 'tool'
              });
            }
          }
...
export function createSessionMonitor(taskId: string, onEvent: (event: any) => void): () => void {
  ...
  fs.readdir(LOG_DIR).then(files => {
    ...
    // Start monitoring
    cleanup = monitorLog(logPath, (event) => {
      try {
        const chunk = streamParser.eventToChunk(event);
        onEvent({
          ...
          event: {
            type: event.type,
            tool: streamParser.isToolUseEvent(event) ? (event as any).tool : undefined,
            content: chunk?.content,
            error: streamParser.isErrorEvent(event) ? (event as any).error : undefined
          }
        });
      } catch (err) {
 ...
 */
async function determineSessionStatus(logFile: string): Promise<'running' | 'completed' | 'failed'> {
  try {
    const PROJECT_ROOT = process.cwd();
