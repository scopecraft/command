...
import { WorktreeService, TaskCorrelationService } from '../../core/worktree/index.js';
 ...
 */
async function handleWorktreeChanges(req: Request, url: URL, worktreeId: string): Promise<Response> {
  try {
    if (!worktreeId) {
      return Response.json({
        success: false,
