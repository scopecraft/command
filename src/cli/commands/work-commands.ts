...
export async function handleWorkCommand(
  ...
  try {
    ...
    if (!taskId) {
      taskId = await selectTaskInteractive(projectRoot);
      if (!taskId) {
    ...
    if (options.docker === false) {
      printSuccess(
        "Running in interactive mode (work command is always interactive)",
      );
    }

    // Create ChannelCoder client and execute
    const client = createChannelCoderClient();
    
    const result = await client.executeInteractive({
      taskId,
      taskInstruction,
      mode,
      additionalPrompt,
    ...
    printError(
      `Work command failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(1);
  }
}
...
async function selectTaskInteractive(
  ...
  try {
    ...
    if (!tasks || tasks.length === 0) {
      printWarning(
        'No current tasks found. Use "sc task list --all" to see all tasks.',
      );
      return undefined;
    }
