...
export function validateStatus(status?: string): string | undefined {
  ...
  if (!isValidStatus(status)) {
    ...
    throw new Error(
      `Invalid status "${status}". Allowed values: ${allowed.join(', ')}`
    );
  }
...
export function validateType(type: string): string {
  if (!isValidType(type)) {
    ...
    throw new Error(
      `Invalid type "${type}". Allowed values: ${allowed.join(', ')}`
    );
  }
...
export function validatePriority(priority?: string): string | undefined {
  ...
  if (!isValidPriority(priority)) {
    ...
    throw new Error(
      `Invalid priority "${priority}". Allowed values: ${allowed.join(', ')}`
    );
  }
...
export function validateWorkflowState(state?: string): string | undefined {
  ...
  if (!isValidWorkflowState(state)) {
    ...
    throw new Error(
      `Invalid workflow state "${state}". Allowed values: ${allowed.join(', ')}`
    );
  }
