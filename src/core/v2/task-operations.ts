...
export async function extractSubtask(
  ...
  try {
    ...
    // Find the subtask
    const subtask = parentResult.data.subtasks.find(st => 
      st.metadata.id === subtaskId || 
      st.metadata.filename.includes(subtaskId)
    );
  ...
  try {
    ...
    if (!sequence) {
      if (options.after) {
        ...
        const afterTask = parentTask.subtasks.find(st =>
          st.metadata.id === options.after ||
          st.metadata.filename.includes(options.after!)
        );
        if (afterTask && afterTask.metadata.sequenceNumber) {
          const afterSeq = parseInt(afterTask.metadata.sequenceNumber, 10);
          sequence = String(afterSeq + 1).padStart(2, '0');
        }
      } else if (options.before) {
        // Find the task to insert before
        const beforeTask = parentTask.subtasks.find(st =>
          st.metadata.id === options.before ||
          st.metadata.filename.includes(options.before!)
        );
        if (beforeTask && beforeTask.metadata.sequenceNumber) {
          const beforeSeq = parseInt(beforeTask.metadata.sequenceNumber, 10);
    ...
    if (options.after || options.before) {
      ...
      if (updatedParent.success && updatedParent.data) {
        ...
        const subtasks = updatedParent.data.subtasks.sort((a, b) => {
          const seqA = parseInt(a.metadata.sequenceNumber || '99', 10);
          const seqB = parseInt(b.metadata.sequenceNumber || '99', 10);
          return seqA - seqB;
        });
...
export async function addSubtask(
  ...
  try {
    ...
    if (options.parallelWith) {
      ...
      const parallelTask = parentTask.subtasks.find(st =>
    ...
    if (!sequence) {
      const sequences = parentTask.subtasks
        .map(st => parseInt(st.metadata.sequenceNumber || '0', 10))
        .filter(seq => seq > 0);
