...
export async function createParentTask(
  ...
  try {
    ...
    const overviewOptions: TaskCreateOptions = {
      ...options,
      instruction: options.instruction || `This is a parent task that will be broken down into child tasks.`,
...
function getNextSequenceNumber(taskFolder: string): string {
  ...
  const sequences = subtasks.map(s => parseInt(s.sequenceNumber, 10));
...
export async function canConvertToParent(
  ...
  return !result.data.metadata.isParentTask && 
