  ...
  async listWorktrees(useCache = true): Promise<Worktree[]> {
    ...
    return this.cacheManager.getOrCompute(cacheKey, async () => {
      try {
        ...
        const worktrees = parsedWorktrees.filter(
          (worktree) => worktree.path && worktree.branch && worktree.headCommit
        ) as Worktree[];
  ...
  async getLastCommit(worktreePath: string, useCache = true): Promise<CommitInfo> {
    ...
    return this.cacheManager.getOrCompute(cacheKey, async () => {
      try {
        ...
        const result = await git.raw(['log', '-1', '--format=%H|%s|%an|%ad']);
        ...
        throw new GitOperationError(
          `Failed to get last commit: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    });
  }
