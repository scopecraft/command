      ...
      if (entry.isDirectory() && !systemDirs.includes(entry.name)) {
        ...
        if (fs.existsSync(phaseInfoPath)) {
          try {
            ...
            console.warn(`Error parsing phase info for ${entry.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
          }
        }
        ...
        try {
          ...
          console.warn(`Error counting tasks for ${entry.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      ...
      if (tasksResult.success && tasksResult.data) {
        for (const task of tasksResult.data) {
          try {
            ...
            console.warn(`Error updating task ${task.metadata.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
          }
        }
      }
...
export async function deletePhase(id: string, options: { force?: boolean } = {}): Promise<OperationResult<void>> {
  try {
    ...
    const hasTasks = entries.some(entry => entry.endsWith('.md') || fs.statSync(path.join(phaseDir, entry)).isDirectory());
