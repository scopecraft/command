...
import { listTasks, getTask } from './task-crud.js';
  ...
  try {
    if (!taskId) {
      ...
      const availableTasks = nonOverviewTasks.filter(task => {
        ...
        if (task.metadata.depends && Array.isArray(task.metadata.depends) && task.metadata.depends.length > 0) {
      ...
      const dependentTasks = tasksResult.data.filter(t => 
        t.metadata.depends && 
