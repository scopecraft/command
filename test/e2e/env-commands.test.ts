  ...
  describe('env create command', () => {
    test('should create environment for simple task', async () => {
      ...
      try {
        ...
        const hasEnvMessage = consoleOutput.some(line => 
    ...
    test('should handle non-existent task', async () => {
      ...
      try {
        ...
        expect(consoleError.some(line => line.includes('Task \'non-existent-task\' not found'))).toBe(true);
      } finally {
        restoreMocks();
      }
    });
    ...
    test('should switch to existing environment', async () => {
      ...
      try {
        ...
        expect(consoleOutput.some(line => line.includes('Switched to existing environment'))).toBe(true);
  ...
  describe('env list command', () => {
    ...
    test('should show no environments when none exist', async () => {
      ...
      try {
        ...
        expect(consoleOutput.some(line => line.includes('No active environments found'))).toBe(true);
        expect(consoleOutput.some(line => line.includes('Create an environment with:'))).toBe(true);
  ...
  describe('env close command', () => {
    test('should require force flag for safety', async () => {
      ...
      try {
        ...
        expect(consoleOutput.some(line => line.includes('Use --force to skip this confirmation'))).toBe(true);
    ...
    test('should handle non-existent environment', async () => {
      ...
      try {
        ...
        expect(consoleError.some(line => line.includes('Task not found:') || line.includes('No environment found for task'))).toBe(true);
      } finally {
        restoreMocks();
      }
    });
