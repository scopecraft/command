...
async function runTests() {
  ...
  try {
    ...
    if (parentResult.success && parentResult.data) {
      ...
      if (getParent.success && getParent.data) {
        ...
        for (const info of subtaskInfo) {
          console.log(`  - ${info.sequenceNumber}: ${info.filename} (parallel: ${info.canRunParallel})`);
        }
      }
      
      // Move parent task
      const moveParent = await v2.moveParentTask(TEST_PROJECT, parentResult.data.metadata.id, 'current');
    ...
    if (parsed) {
      console.log(`✓ Parsed ID: name=${parsed.descriptiveName}, date=${parsed.dateCode}, suffix=${parsed.randomSuffix}`);
    }
