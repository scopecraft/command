...
describe('Work Command', () => {
  ...
  it('should show error when no tasks exist', () => {
    ...
    const output = execSync(
      `bun run ${CLI_PATH} --root-dir ${TEST_DIR} work --help 2>&1`,
      { encoding: 'utf8' }
    );
  ...
  it('should require valid task ID when provided', async () => {
    try {
      execSync(
        `bun run ${CLI_PATH} --root-dir ${TEST_DIR} work non-existent-task 2>&1`,
  ...
  it.skip('should handle task with parent correctly', async () => {
    ...
    const parentResult = await createTask(TEST_DIR, {
      ...
      location: 'current',
      id: '_overview'
    }, undefined, 'parent-feat-01');
    
    expect(parentResult.success).toBe(true);
    
    // Create a subtask
    const subtaskResult = await createTask(TEST_DIR, {
  ...
  it('should accept mode override option', () => {
    try {
      const output = execSync(
        `bun run ${CLI_PATH} --root-dir ${TEST_DIR} work --help`,
        { encoding: 'utf8' }
      );
  ...
  it('should have short alias', () => {
    try {
      const output = execSync(
        `bun run ${CLI_PATH} --root-dir ${TEST_DIR} w --help`,
        { encoding: 'utf8' }
      );
