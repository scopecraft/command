...
import type { ConfigurationManager } from '../../../../src/core/config/configuration-manager.js';
  ...
  describe('getWorktreePath', () => {
    test('should return correct worktree path for task', async () => {
      ...
      expect(worktreePath).toBe(
        '/Users/test/projects/my-awesome-project.worktrees/implement-auth-05A'
      );
    });
    ...
    test('should handle task IDs with special characters', async () => {
      ...
      const worktreePath = await resolver.getWorktreePath(taskId);
      expect(worktreePath).toBe(
        '/Users/test/projects/my-awesome-project.worktrees/fix-bug_123-06B'
      );
    });
    ...
    test('should throw error for null task ID', async () => {
      await expect(resolver.getWorktreePath(null as string)).rejects.toThrow(EnvironmentError);
      await expect(resolver.getWorktreePath(null as string)).rejects.toMatchObject({
        code: EnvironmentErrorCodes.INVALID_TASK_ID,
      });
    });

    test('should throw error for undefined task ID', async () => {
      await expect(resolver.getWorktreePath(undefined as string)).rejects.toThrow(EnvironmentError);
      await expect(resolver.getWorktreePath(undefined as string)).rejects.toMatchObject({
        code: EnvironmentErrorCodes.INVALID_TASK_ID,
      });
    });
