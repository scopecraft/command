...
export function FeatureFormView({ featureId, isEdit = false }: FeatureFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate(
          isEdit ? routes.featureDetail(featureData.id.replace('FEATURE_', '')) : routes.home
        );
      } else {
  ...
  const handleCancel = () => {
    navigate(
      isEdit && featureId ? routes.featureDetail(featureId.replace('FEATURE_', '')) : routes.home
    );
  };

  return (
    <div className="container mx-auto p-4 max-w-3xl">
      <h1 className="text-xl font-semibold mb-6">{isEdit ? 'Edit Feature' : 'Create Feature'}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Human-readable title for the feature.</p>
        </div>
